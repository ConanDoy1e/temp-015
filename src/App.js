...
// ─── Mock Data (100건 — 무연고 실데이터 5건 + 일반 95건) ─────
const mockData = [
  // ━━━ 무연고 사망자 공고 (실제 형식 참고, isRealData: true) ━━━
  {
    id: 1001,
    isRealData: true,
    name: "성명불상",
    ...
    region: "서울 영등포구",
    date: "2026-03-10",
    funeral: "서울시립승화원 (서울특별시 관할)",
    contact: "영등포구청 복지정책과 02-2670-3114",
    age: 70,
    discoveryLocation: "서울 영등포구 대림동 소재 고시원",
    noticeNumber: "영등포구 고시 제2026-221호",
    message: "2026년 2월 28일 영등포구 대림동 소재 고시원에서 발견. 신원 확인 불가. 무연고 사망자로 공고하며, 연고자는 공고일로부터 60일 이내 관할 구청에 연락 바랍니다.",
  },
  {
    id: 1002,
    isRealData: true,
    ...
    region: "부산 동구",
    date: "2026-03-07",
    funeral: "부산시립장례식장 (부산광역시 관할)",
    contact: "부산 동구청 주민복지과 051-440-4000",
    age: 65,
    discoveryLocation: "부산 동구 범일동 소재 쪽방",
    noticeNumber: "부산 동구 고시 제2026-87호",
    message: "2026년 2월 25일 부산 동구 범일동 소재 쪽방에서 사망한 채 발견. 주민등록 말소 상태. 연고자 확인 요청 공고이며, 60일 이내 동구청으로 연락 바랍니다.",
  },
  {
    id: 1003,
    isRealData: true,
    ...
    region: "인천 남동구",
    date: "2026-03-05",
    funeral: "인천시립장례식장 (인천광역시 관할)",
    contact: "남동구청 사회복지과 032-453-2114",
    age: 78,
    discoveryLocation: "인천 남동구 만수동 소재 임대아파트",
    noticeNumber: "남동구 고시 제2026-156호",
    message: "2026년 2월 21일 인천 남동구 만수동 임대아파트에서 사망 상태로 발견. 독거 생활 중이었으며 가족 연락 두절 상태. 연고자는 공고일로부터 60일 이내 구청에 연락 바랍니다.",
  },
  {
    id: 1004,
    isRealData: true,
    ...
    region: "대구 북구",
    date: "2026-03-03",
    funeral: "대구시립장례식장 (대구광역시 관할)",
    contact: "대구 북구청 복지정책과 053-665-2114",
    age: 55,
    discoveryLocation: "대구 북구 노원동 소재 공원 인근",
    noticeNumber: "대구 북구 고시 제2026-63호",
    message: "2026년 2월 18일 대구 북구 노원동 소재 공원 인근에서 발견. 소지품 내 신분증 없으며 지문 조회 결과 신원 미상. 연고자는 60일 이내 북구청으로 연락 바랍니다.",
  },
  {
    id: 1005,
    isRealData: true,
    ...
    region: "경기 수원시",
    date: "2026-02-28",
    funeral: "수원시립장례식장 (수원시 관할)",
    contact: "수원시 팔달구청 복지과 031-228-7114",
    age: 82,
    discoveryLocation: "수원시 팔달구 인계동 소재 단독주택",
    noticeNumber: "팔달구 고시 제2026-41호",
    message: "2026년 2월 14일 수원시 팔달구 인계동 단독주택에서 사망 상태로 발견. 오랜 기간 이웃과 교류 없이 독거 생활. 연고자는 공고일로부터 60일 이내 구청에 연락 바랍니다.",
  },
  // ━━━ 일반 부고 데이터 (id 1–95) ━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
function PopularKeywords({ onKeywordClick }) {
  return (
    <div style={{ backgroundColor: C.raised, borderColor: C.border }} className="rounded-lg border p-5 mb-8">
      <div className="flex items-center gap-2.5 mb-5">
        <span style={{ color: C.accent }}><TrendIcon /></span>
        <h2 style={{ color: C.txt1 }} className="text-sm font-semibold tracking-tight">많이 검색된 키워드</h2>
        <span style={{ color: C.txt3, backgroundColor: C.overlay, borderColor: C.borderSoft }} className="ml-auto text-[11px] font-medium px-2 py-0.5 rounded-full border">TOP 10</span>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-0.5">
        {popularKeywords.map((item, idx) => (
          ...
          >
            <span className="rank-badge" style={{ backgroundColor: idx < 3 ? C.accentMuted : C.overlay, color: idx < 3 ? C.accent : C.txt3 }}>{idx + 1}</span>
            <span style={{ color: C.txt1 }} className="text-[13px] font-medium truncate">{item.keyword}</span>
            <span style={{ color: C.txt3 }} className="ml-auto text-[11px] tabular-nums font-medium">{formatCount(item.count)}</span>
          </button>
        ))}
      </div>
    </div>
...
function Pagination({ currentPage, totalPages, onPageChange }) {
  ...
  const btnStyle = (active) => ({
    height: 32, padding: "0 12px", fontSize: 13, borderRadius: 6,
  ...
  const NavBtn = ({ disabled, onClick, children }) => (
    <button onClick={onClick} disabled={disabled}
      style={{ ...btnStyle(false), opacity: disabled ? 0.25 : 1, pointerEvents: disabled ? "none" : "auto" }}
      onMouseEnter={(e) => { if (!disabled) { e.currentTarget.style.backgroundColor = C.hovBg; e.currentTarget.style.color = C.txt1; } }}
      onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "transparent"; e.currentTarget.style.color = C.txt2; }}
    >{children}</button>
  );

  return (
    <div className="flex items-center justify-center gap-1.5 mt-10 flex-wrap">
      <NavBtn disabled={currentPage === 1} onClick={() => onPageChange(currentPage - 1)}>&lsaquo; 이전</NavBtn>
      {start > 1 && (<><button style={btnStyle(false)} onClick={() => onPageChange(1)}>1</button>{start > 2 && <span style={{ color: C.txt3, padding: "0 4px", fontSize: 12 }}>...</span>}</>)}
      {pages.map((page) => (
        <button key={page} style={{ ...btnStyle(page === currentPage), width: 32, padding: 0 }} onClick={() => onPageChange(page)}
          onMouseEnter={(e) => { if (page !== currentPage) { e.currentTarget.style.backgroundColor = C.hovBg; e.currentTarget.style.color = C.txt1; } }}
          onMouseLeave={(e) => { if (page !== currentPage) { e.currentTarget.style.backgroundColor = "transparent"; e.currentTarget.style.color = C.txt2; } }}
        >{page}</button>
      ))}
      {end < totalPages && (<>{end < totalPages - 1 && <span style={{ color: C.txt3, padding: "0 4px", fontSize: 12 }}>...</span>}<button style={btnStyle(false)} onClick={() => onPageChange(totalPages)}>{totalPages}</button></>)}
      <NavBtn disabled={currentPage === totalPages} onClick={() => onPageChange(currentPage + 1)}>다음 &rsaquo;</NavBtn>
    </div>
  );
}
...
function DetailModal({ person, onClose }) {
  ...
  const isReal = person.isRealData;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm px-4 backdrop-enter"
      style={{ backgroundColor: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div className="rounded-xl max-w-lg w-full max-h-[90vh] overflow-y-auto modal-enter"
        style={{ backgroundColor: C.raised, border: `1px solid ${C.border}`, boxShadow: "0 24px 80px rgba(0,0,0,0.7)" }}
            ...
            <div>
              <div className="flex items-center gap-2.5">
                <h2 style={{ color: C.txt1 }} className="text-lg font-bold">
                  {isReal ? "" : "故 "}{person.name}
                </h2>
                {isReal && <UnclaimedBadge />}
              </div>
              <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                {isReal && person.gender && (
                  <><span style={{ color: C.txt2 }} className="text-[13px]">{person.gender}</span>
                  <span style={{ backgroundColor: C.txt3 }} className="w-1 h-1 rounded-full" /></>
                )}
                <span style={{ color: C.txt2 }} className="text-[13px]">{isReal ? `추정 ` : ``}향년 {person.age}세</span>
                <span style={{ backgroundColor: C.txt3 }} className="w-1 h-1 rounded-full" />
                <span style={{ color: C.txt3 }} className="text-[13px]">{formatDate(person.date)}</span>
              </div>
            </div>
            <button onClick={onClose} style={{ color: C.txt3 }} className="p-1 rounded-md transition-colors"
              onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = C.hovBg; e.currentTarget.style.color = C.txt1; }}
              onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "transparent"; e.currentTarget.style.color = C.txt3; }}
              aria-label="닫기"><CloseIcon /></button>
          </div>
        </div>

        {/* Body */}
        <div className="px-6 py-5 space-y-5">
          <p style={{ color: C.txt2, borderColor: "rgba(94,92,230,0.4)" }} className="text-[14px] leading-relaxed border-l-2 pl-4">
            {person.message}
          </p>

          {/* 무연고 전용 정보 */}
          {isReal && person.discoveryLocation && (
            <div style={{ backgroundColor: C.overlay, border: `1px solid ${C.borderSoft}` }} className="rounded-lg p-4 space-y-2">
              <h3 style={{ color: C.txt1 }} className="text-[13px] font-semibold">발견 정보</h3>
              <div className="flex items-start gap-2">
                <span style={{ color: C.txt3 }} className="text-[12px] min-w-[56px]">발견장소</span>
                <span style={{ color: C.txt2 }} className="text-[13px]">{person.discoveryLocation}</span>
              </div>
              {person.noticeNumber && (
                <div className="flex items-start gap-2">
                  <span style={{ color: C.txt3 }} className="text-[12px] min-w-[56px]">공고번호</span>
                  <span style={{ color: C.txt2 }} className="text-[13px]">{person.noticeNumber}</span>
                </div>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {!isReal && <InfoItem label="학교" value={person.school} />}
            {!isReal && <InfoItem label="직장" value={person.company} />}
            <InfoItem label={isReal ? "관할 지역" : "거주지"} value={person.region} />
            <InfoItem label={isReal ? "공고일" : "부고일"} value={formatDate(person.date)} />
          </div>

          {/* 장례/안치 정보 */}
          <div style={{ backgroundColor: C.overlay, border: `1px solid ${C.borderSoft}` }} className="rounded-lg p-4 space-y-3">
            <h3 style={{ color: C.txt1 }} className="text-[13px] font-semibold">
              {isReal ? "안치 및 연락처 정보" : "장례식장 정보"}
            </h3>
            <div className="space-y-2">
              <div className="flex items-start gap-2">
                <span style={{ color: C.txt3 }} className="text-[12px] min-w-[40px]">{isReal ? "안치" : "장소"}</span>
        ...
        <div className="px-6 pb-5">
          <button onClick={onClose}
            style={{ backgroundColor: C.overlay, color: C.txt2, border: `1px solid ${C.borderSoft}` }}
            className="w-full py-2.5 rounded-lg text-[13px] font-medium transition-all"
            onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = C.hovBg; e.currentTarget.style.color = C.txt1; e.currentTarget.style.borderColor = C.border; }}
            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = C.overlay; e.currentTarget.style.color = C.txt2; e.currentTarget.style.borderColor = C.borderSoft; }}
          >닫기</button>
        </div>
      </div>
    </div>
...
function InfoItem({ label, value }) {
  return (
    <div style={{ backgroundColor: C.overlay, border: `1px solid ${C.borderSoft}` }} className="rounded-lg p-3">
...
function ObituaryCard({ person, onClick }) {
  ...
  return (
    <button onClick={onClick}
      ...
      onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = C.hovBg; }}
      onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = C.raised; }}
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p style={{ color: C.txt3 }} className="text-[12px] mt-1">
            {formatDate(person.date)}{isReal && person.gender ? ` · ${person.gender}` : ""}
          </p>
        </div>
        <span style={{ backgroundColor: C.accentMuted, color: C.accent }} className="text-[11px] px-2.5 py-1 rounded-full font-semibold shrink-0">
          {isReal ? "추정 " : "향년 "}{person.age}세
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 text-[12px]">
        {isReal ? (
          <>
            <span style={{ backgroundColor: C.overlay, color: C.txt2, border: `1px solid ${C.borderSoft}` }} className="px-2.5 py-1 rounded-md">{person.region}</span>
            {person.discoveryLocation && (
              <span style={{ backgroundColor: C.overlay, color: C.txt2, border: `1px solid ${C.borderSoft}` }} className="px-2.5 py-1 rounded-md truncate max-w-[280px]">{person.discoveryLocation}</span>
            )}
          </>
        ) : (
          [person.school, person.company, person.region].map((tag) => (
            <span key={tag} style={{ backgroundColor: C.overlay, color: C.txt2, border: `1px solid ${C.borderSoft}` }} className="px-2.5 py-1 rounded-md">{tag}</span>
          ))
        )}
      </div>
      ...
      <div style={{ color: C.accent }} className="flex items-center gap-1 mt-3 text-[12px] font-medium transition-colors">
        <span>상세 정보 보기</span>
        <ChevronRight />
      </div>
    </button>
...
export default function App() {
  ...
  const filteredData = useMemo(() => {
    ...
    // 무연고 데이터는 항상 최상단, 나머지는 날짜순
    const realData = mockData.filter((p) => p.isRealData);
    const normalData = [...mockData.filter((p) => !p.isRealData)].sort(
      (a, b) => new Date(b.date) - new Date(a.date)
    );
    const sorted = [...realData, ...normalData];
    ...
    return sorted.filter(
      (p) =>
        ...
        (p.discoveryLocation && p.discoveryLocation.toLowerCase().includes(q)) ||
        (p.noticeNumber && p.noticeNumber.toLowerCase().includes(q))
    );
  }, [query]);
  ...
  const handleQueryChange = (value) => { setQuery(value); setCurrentPage(1); };
  const handleKeywordClick = (keyword) => { handleQueryChange(keyword); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  return (
    <div style={{ backgroundColor: C.bg, color: C.txt1, minHeight: "100vh" }} className="font-sans">
      ...
      <section className="pt-16 sm:pt-20 pb-10 px-5">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="mt-8 relative max-w-xl mx-auto">
            <span style={{ color: C.txt3 }} className="absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none"><SearchIcon /></span>
            <input type="text" value={query} onChange={(e) => handleQueryChange(e.target.value)}
              ...
              style={{ backgroundColor: C.raised, border: `1px solid ${C.border}`, color: C.txt1 }} />
            {query && (
              <button onClick={() => handleQueryChange("")} style={{ color: C.txt3 }}
                className="absolute right-3.5 top-1/2 -translate-y-1/2 p-0.5 rounded transition-colors"
                onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = C.hovBg; e.currentTarget.style.color = C.txt1; }}
                onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "transparent"; e.currentTarget.style.color = C.txt3; }}
                aria-label="검색어 지우기"><CloseIcon /></button>
            )}
          </div>
        </div>
      </section>
      ...
      <main className="max-w-3xl mx-auto px-5 pb-12">
        {!isSearching && currentPage === 1 && <PopularKeywords onKeywordClick={handleKeywordClick} />}

        <div className="flex items-center justify-between mb-5">
          <p style={{ color: C.txt2 }} className="text-[13px]">
            {isSearching ? (
              <><span style={{ color: C.accent }} className="font-semibold">&lsquo;{query}&rsquo;</span>{" 검색 결과 "}<span style={{ color: C.txt1 }} className="font-semibold">{filteredData.length}건</span></>
            ) : (
              <>전체 부고 <span style={{ color: C.txt1 }} className="font-semibold">{filteredData.length}건</span></>
            )}
          </p>
          {totalPages > 1 && <p style={{ color: C.txt3 }} className="text-[12px]">{currentPage} / {totalPages} 페이지</p>}
        </div>

        {isSearching && filteredData.length === 0 && (
          <div className="text-center py-20">
            <div style={{ backgroundColor: C.raised, border: `1px solid ${C.borderSoft}`, color: C.txt3 }} className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4"><SearchIcon /></div>
            <p style={{ color: C.txt2 }} className="text-[15px] font-medium">&lsquo;{query}&rsquo;에 대한 검색 결과가 없습니다</p>
            <p style={{ color: C.txt3 }} className="text-[13px] mt-1.5">다른 키워드로 검색해보세요</p>
          </div>
        )}
        ...
        {paginatedData.length > 0 && (
          <div className="grid gap-3">
            {paginatedData.map((person) => (
              <ObituaryCard key={person.id} person={person} onClick={() => setSelected(person)} />
            ))}
          </div>
        )}
      ...
      <footer style={{ borderTop: `1px solid ${C.borderSoft}` }} className="text-center py-6">
        <p style={{ color: C.txt3 }} className="text-[12px]">&copy; 2026 Last Link &middot; 마지막 인사를 전하는 곳</p>
      </footer>
